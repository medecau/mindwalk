...
- `internal/citymap` builds deterministic layouts from repository contents: a quadtree "organic city" centered on the repo root, with a "+" cross splitting each level into up to four weighted, burrow-biased quadrant districts around a central plaza for that node's own loose files.
